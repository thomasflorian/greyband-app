...
export default function EditProfileScreen({ navigation }) {
    ...
    const renderItem = ({item:{label, value, display, key}, index}) => {
        return (
            <View style={{flexDirection: "row", width:"90%", height: 85, marginHorizontal: "5%"}}>
                <View style={{flex: 3}}>
                    <Text style={{ color: "#D64F27", fontSize: 20}}>{label}</Text>
                    <TextInput onFocus={() => flatlistRef.scrollToIndex({index})} onChangeText={(newValue) => handleValueChange(newValue, key)} value={value} style={{...styles.box, color: "#D64F27", paddingHorizontal: 10}}></TextInput>
                </View>
                <View style={{flex: 1, maxWidth:70, alignItems: "center"}}>
                    <Text style={{ color: "#D64F27", fontSize: 20}}>{index == 0 ? "Display":" "}</Text>
                    <TouchableOpacity onPress={() => handleDisplayToggle(key)} style={{...styles.box, backgroundColor: display ? '#D64F27':'#0F2138'}}></TouchableOpacity>
                </View>
            </View>
        )
    }
            ...
            <View style={styles.box}>
                <View style={{width: "100%", flex: 1}}>
                    <View style={{flexDirection: "row", alignItems: "center", marginHorizontal: 20, marginVertical: 10}}>
                        <TouchableOpacity style={{flex:1}} onPress={() => navigation.navigate("Profile")}><Icon name='arrow-back-ios' color='#D64F27' size={30} /></TouchableOpacity>
                        <Text style={{color: '#D64F27', fontSize: 24, fontWeight: "700", flex: 4, textAlign: "center"}}>Edit Profile</Text>
